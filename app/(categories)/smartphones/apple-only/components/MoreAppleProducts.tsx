...
export default function MoreAppleProducts() {
  return (
    <section className="mb-10 sm:mb-16">
      <style>{`
        @keyframes mapFadeUp{from{opacity:0;transform:translateY(15px)}to{opacity:1;transform:translateY(0)}}
        .map-header{animation:mapFadeUp .5s ease both}
        .map-card-0{animation:mapFadeUp .5s ease both}
        .map-card-1{animation:mapFadeUp .5s .12s ease both}
        .map-card-2{animation:mapFadeUp .5s .24s ease both}
      `}</style>

      <div className="map-header flex items-center gap-3 mb-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-5">
        {appleProducts.map((product, i) => (
          <div key={product.id} className={`map-card-${i}`}>
            <Link
              href={product.href}
              className="group block rounded-3xl overflow-hidden border border-[#EBE6E2] hover:border-[#DFC4A4]/50 transition-all duration-300 hover:shadow-xl"
